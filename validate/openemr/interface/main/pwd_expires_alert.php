...
if (($pwd_expires == "0000-00-00") or ($pwd_expires == "")) {
    ...
    $case = "alertmsg1";
} elseif ((strtotime($current_date) > strtotime($pwd_expires)) && ($grace_time != "") &&
    ...
    $case = "alertmsg1";
} elseif (strtotime($pwd_expires) == strtotime($current_date)) {
    // Display warning if password expires on current day
    $msg_alert = xl("Your Password Expires today. Please change your password.");
    $case = "alertmsg2";
} elseif ((strtotime($current_date) >= strtotime($pwd_alert)) && strtotime($pwd_alert) != "") {
