...
class EmailMessage
{
    ...
    public static $RECIPIENT_TYPE_TO = "";
    public static $RECIPIENT_TYPE_BCC = "BCC";
    public static $RECIPIENT_TYPE_CC = "CC";

    /** @var bool this controls the default value for DecodeEntities */
    public static $DECODE_HTML_ENTITIES = true;
     ...
     */
    public function AddRecipient($email, $name = "", $recipientType = "")
    {
        if ($this->DecodeEntities) {
            $email = $this->DecodeEntities($email);
            $name = $this->DecodeEntities($name);
