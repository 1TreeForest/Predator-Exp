...
//        die;
    ...
    public function searchGoods($words, $order = 'asc', $page, $num, $user_id)
    {
        ...
        if ($words == '') { // 没有关键字的时候
            ...
            if ($num == '' && $order == '') {
                $sql = "SELECT goods_id AS id,goods_name AS name,shop_price AS retail_price,
                        goods_thumb AS list_pic_url FROM ecs_goods ORDER BY shop_price 
                        LIMIT {$offset},{$page_size}";
            } else if ($num == '') {
                $sql = "SELECT goods_id AS id,goods_name AS name,shop_price AS retail_price,
                       goods_thumb AS list_pic_url FROM ecs_goods ORDER BY shop_price $order
                        LIMIT {$offset},{$page_size}";
            } else {
                $sql = "select goods_id AS id,goods_name AS name,shop_price AS retail_price,
                       goods_thumb AS list_pic_url FROM ecs_goods order by sales_volume_count $num limit $offset,$page_size";
            }
        ...
        $sql = "SELECT goods_id AS id,goods_name AS name,shop_price AS retail_price,goods_thumb AS list_pic_url FROM ecs_goods WHERE is_on_sale = 1 and goods_name LIKE '%{$words}%' ORDER BY shop_price {$order} LIMIT {$offset},{$page_size}";
        $res = $this->getORM()->queryRows($sql);
