...
class Order extends NotORM
{
    ...
    public function getAllOrderNum($user_id)
    {
        ...
        // 获得当前用户的待收货的订单
        $sql = "SELECT COUNT(order_id) AS num FROM ecs_order_info WHERE shipping_status = 1 AND user_id = {$user_id}";
        $shipping_status = $this->getORM()->queryRows($sql);

        // 获得当前用户的退货数
        $sql = "SELECT COUNT(order_id) AS num FROM ecs_order_info WHERE order_status = 4 AND shipping_status = 4 AND user_id = {$user_id}";
        $order_status = $this->getORM()->queryRows($sql);

        // 获得当前用户的待发货数
        $sql = "SELECT COUNT(order_id) AS num FROM ecs_order_info WHERE shipping_status = 0 AND user_id = {$user_id}";
