...
class Comment extends NotORM
{
    ...
    public function getGoodsComment($goods_id, $page)
    {
        ...
        // 获取当前商品的评论信息
        $sql = "select avg(comment_rank) as score from ecs_comment where id_value = " . $goods_id . ""; // 获取当前商品的总平均分
        $score = $this->getORM()->queryRow($sql);

        $sql = "select user_name,comment_rank,add_time,content from ecs_comment where id_value = " . $goods_id . " order by add_time desc limit {$page},{$page_size} ";
